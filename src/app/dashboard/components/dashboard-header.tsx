...
const VALID_SECTIONS = ['users', 'curriculum', 'statistics', 'groups'] as const;
...
export function DashboardHeader() {
  ...
  const activeTab = (
    VALID_SECTIONS.includes(section as any) ? section : 'users'
  ...
  return (
    <div className="mb-8 animate-in fade-in duration-500">
      <h1 className="text-3xl font-bold bg-linear-to-r from-primary to-secondary bg-clip-text text-transparent mb-2 leading-[3]">
        {labels.dashboard.title[activeTab] ?? labels.dashboard.title.users}
      </h1>
    </div>
  );
}
