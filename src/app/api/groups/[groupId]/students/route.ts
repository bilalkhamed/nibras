...
export async function POST(
  ...
  const studentResult = await getUserWithRoleAndCohort(body.studentId);

  if (!studentResult.success) {
    if (studentResult.error.type === 'not-found') {
      return NextResponse.json(
    ...
    return NextResponse.json(
      { error: studentResult.error.type },
      { status: studentResult.error.statusCode },
    );
  }
